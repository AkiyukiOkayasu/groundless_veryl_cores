...
REQUIRED_COLUMNS = {
    ...
    "cubic_minus_hold_bits",
    "cubic_minus_linear_bits",
}
        ...
        for line_number, row in enumerate(reader, start=2):
            try:
                parsed = {
                    ...
                    "cubic": decode_twos_complement(row["cubic_bits"], SAMPLE_WIDTH),
                    "linear_difference": decode_twos_complement(
                        row["linear_minus_hold_bits"], DIFFERENCE_WIDTH
                    ),
                    "cubic_difference": decode_twos_complement(
                        row["cubic_minus_hold_bits"], DIFFERENCE_WIDTH
                    ),
                    "cubic_linear_difference": decode_twos_complement(
                        row["cubic_minus_linear_bits"], DIFFERENCE_WIDTH
                    ),
                }
                ...
                if parsed["cubic_difference"] != parsed["cubic"] - parsed["hold"]:
                    raise ValueError("cubic_minus_hold_bitsがcubic_bits - zero_order_hold_bitsと不一致")
                if parsed["cubic_linear_difference"] != parsed["cubic"] - parsed["linear"]:
                    raise ValueError("cubic_minus_linear_bitsがcubic_bits - linear_bitsと不一致")
                rows.append(parsed)
...
def difference_summary(case: int, rows: list[dict[str, int]]) -> dict[str, int | float]:
    ...
    for name in ("linear_difference", "cubic_difference", "cubic_linear_difference"):
...
def print_text(result: dict[str, object]) -> None:
    ...
    comparison_names = {
        ...
        "cubic_difference": "cubic-hold",
        "cubic_linear_difference": "cubic-linear",
    }
    ...
    print(
        "frequency_input_fs hold_db linear_db cubic_db "
        "linear_minus_hold_db cubic_minus_hold_db cubic_minus_linear_db"
    )
    ...
    cubic_response = impulse["frequency_response_db_relative_to_dc"]["cubic"]  # type: ignore[index]
    for frequency in hold_response:
        hold_db = hold_response[frequency]
        linear_db = linear_response[frequency]
        cubic_db = cubic_response[frequency]
        linear_delta = None if hold_db is None or linear_db is None else linear_db - hold_db
        cubic_delta = None if hold_db is None or cubic_db is None else cubic_db - hold_db
        cubic_linear_delta = (
            None if linear_db is None or cubic_db is None else cubic_db - linear_db
        )
        print(
            f"{frequency} "
            f"{hold_db if hold_db is not None else 'n/a'} "
            f"{linear_db if linear_db is not None else 'n/a'} "
            f"{cubic_db if cubic_db is not None else 'n/a'} "
            f"{linear_delta if linear_delta is not None else 'n/a'} "
            f"{cubic_delta if cubic_delta is not None else 'n/a'} "
            f"{cubic_linear_delta if cubic_linear_delta is not None else 'n/a'}"
        )
